...
-- 经纪人基础信息
CREATE TABLE IF NOT EXISTS dw_temp_angejia.dw_broker_summary_basis (
  broker_id string,
  broker_name string,
...
INSERT OVERWRITE TABLE
  dw_temp_angejia.dw_broker_summary_basis
SELECT
...
-- 经纪人中心表
CREATE TABLE IF NOT EXISTS dw_temp_angejia.dw_broker_summary_agent_info (
  broker_id string,
  agent_name string,
  company_name string,
  label_name string
...
INSERT OVERWRITE TABLE
  dw_temp_angejia.dw_broker_summary_agent_info
SELECT
  bs.user_id AS broker_id,
...
-- 用户 app 信息
CREATE TABLE IF NOT EXISTS dw_temp_angejia.dw_user_summary_app_info (
  user_id  string,
  app_name  string
);

INSERT OVERWRITE TABLE
  dw_temp_angejia.dw_user_summary_app_info
SELECT
  user_id,
  app_name
FROM
...
-- 经纪人电话表
CREATE TABLE IF NOT EXISTS dw_temp_angejia.dw_broker_summary_call_info (
  broker_id string,
...
INSERT OVERWRITE TABLE
  dw_temp_angejia.dw_broker_summary_call_info
SELECT
  bs.user_id AS broker_id,
  t_1.num,
  t_2.num,
...
-- 带看量
CREATE TABLE IF NOT EXISTS dw_temp_angejia.dw_broker_summary_visit_info (
  broker_id string,
  visit_dkl string,
  visit_fq_dkl string
);
...
INSERT OVERWRITE TABLE
  dw_temp_angejia.dw_broker_summary_visit_info
SELECT
...
-- 跟进量
CREATE TABLE IF NOT EXISTS dw_temp_angejia.dw_broker_summary_followup_info (
  broker_id string,
...
INSERT OVERWRITE TABLE
  dw_temp_angejia.dw_broker_summary_followup_info
SELECT
  bs.user_id AS broker_id,
  t_1.followup_fdgjl,
  t_2.followup_khgjl,
...
-- 服务相关
CREATE TABLE IF NOT EXISTS dw_temp_angejia.dw_broker_summary_service_info (
  broker_id string,
...
INSERT OVERWRITE TABLE
  dw_temp_angejia.dw_broker_summary_service_info
SELECT
  bs.user_id AS broker_id,
  t_1.num,
  t_2.num,
...
-- 客户相关
CREATE TABLE IF NOT EXISTS dw_temp_angejia.dw_broker_summary_demand_info (
  broker_id string,
...
INSERT OVERWRITE TABLE
  dw_temp_angejia.dw_broker_summary_demand_info
SELECT
  bs.user_id AS broker_id,
  t_1.num,
  t_2.num
...
-- 房源相关 start
CREATE TABLE IF NOT EXISTS dw_temp_angejia.dw_broker_summary_inventory_info (
  broker_id string,
...
INSERT OVERWRITE TABLE
  dw_temp_angejia.dw_broker_summary_inventory_info
SELECT
  bs.user_id AS broker_id,
  t_1.num,
  t_2.num,
...
create table dw_temp_angejia.swan_genjin_commission_daily as
select a.broker_uid,c.phone,to_date(a.create_at) as cal_dt,a.create_at
from db_sync.angejia__commission_followup a
left outer join db_sync.angejia__commission b on a.commission_id=b.id
left outer join db_sync.angejia__user_phone c on b.seller_uid=c.user_id
where to_date(a.create_at)= ${dealDate}
and a.broker_uid not in (0,3,4)
;
-- 买家跟进
drop table if exists dw_temp_angejia.swan_genjin_buyer_daily;
create table dw_temp_angejia.swan_genjin_buyer_daily as
select a.broker_uid,b.phone,to_date(a.create_at) as cal_dt,a.create_at
from db_sync.angejia__buyer_followup a
left outer join db_sync.angejia__user_phone b on a.buyer_uid=b.user_id
where to_date(a.create_at)= ${dealDate}
and a.broker_uid not in (0,3,4)
;
-- 房源跟进
...
select a.broker_uid,d.phone,to_date(a.create_at) as cal_dt,a.create_at
from db_sync.angejia__inventory_followup a
left outer join db_sync.property__inventory b on a.inventory_id=b.id
left outer join db_sync.angejia__commission c on b.id=c.inventory_id
left outer join db_sync.angejia__user_phone d on c.seller_uid=d.user_id  
where to_date(a.create_at)= ${dealDate}
and a.broker_uid not in (0,3,4)
;
-- 所有跟进
drop table if exists dw_temp_angejia.swan_genjin_daily_20150504;
create table dw_temp_angejia.swan_genjin_daily_20150504 as
select t.broker_uid,t.phone,t.cal_dt,t.create_at,concat(t.phone,t.create_at) as genjin_detail
from
(
select * from dw_temp_angejia.swan_genjin_commission_daily  
union all
...
create table dw_temp_angejia.swan_laidian_id_daily_20150504 as
select a.called_uid as broker_uid,a.caller_uid,a.caller,b.user_id,start_at,concat(a.caller,start_at) as call_detail,to_date(start_at) as cal_dt
...
and call_type =2
and called_uid not in (0,3,4)
and keep_time>0
and to_date(start_at)= ${dealDate}
;

drop table if exists dw_temp_angejia.swan_laidian_genjin_daily_20150504;
create table dw_temp_angejia.swan_laidian_genjin_daily_20150504 as
select
       t.name
      ,t.broker_uid
...
from dw_temp_angejia.swan_laidian_id_daily_20150504 a
join dw_temp_angejia.swan_genjin_daily_20150504 b
on a.broker_uid=b.broker_uid
and a.caller=b.phone
and a.cal_dt=b.cal_dt
join db_sync.angejia__broker c
on a.broker_uid=c.user_id
where c.city_id<>3
and c.status=2
group by a.broker_uid,c.name
)t;
...
--- 微聊基础信息表 汇总每天的微聊信息 剔除无效信息 start ---
...
-- 合并临时表指标
drop table if exists dw_temp_angejia.dw_broker_summary_wechat_message_daily;

create table dw_temp_angejia.dw_broker_summary_wechat_message_daily
as
select  
 a.broker_id
...
left outer join dw_temp_angejia.wechat_effect_info_day_tmp b
  on a.broker_id = b.broker_id
 and a.cal_dt = b.cal_dt;

--- 微聊基础信息表 汇总每天的微聊信息 剔除无效信息 end ---
...
LEFT JOIN
  dw_temp_angejia.dw_broker_summary_basis AS ds_broker_basis
ON
  bs.user_id = ds_broker_basis.broker_id


LEFT JOIN
  dw_temp_angejia.dw_broker_summary_agent_info AS ds_broker_agent_info
ON
  bs.user_id = ds_broker_agent_info.broker_id


LEFT JOIN
  dw_temp_angejia.dw_user_summary_app_info AS ds_user_app_info
ON
  bs.user_id = ds_user_app_info.user_id


LEFT JOIN
  dw_temp_angejia.dw_broker_summary_call_info AS ds_broker_call_info
ON
  bs.user_id = ds_broker_call_info.broker_id


LEFT JOIN
  dw_temp_angejia.dw_broker_summary_visit_info AS ds_broker_visit_info
ON
  bs.user_id = ds_broker_visit_info.broker_id


LEFT JOIN
  dw_temp_angejia.dw_broker_summary_followup_info AS ds_broker_followup_info
ON
  bs.user_id = ds_broker_followup_info.broker_id


LEFT JOIN
  dw_temp_angejia.dw_broker_summary_service_info AS ds_broker_service_info
ON
  bs.user_id = ds_broker_service_info.broker_id


LEFT JOIN
  dw_temp_angejia.dw_broker_summary_demand_info AS ds_broker_demand_info
ON
  bs.user_id = ds_broker_demand_info.broker_id


LEFT JOIN
  dw_temp_angejia.dw_broker_summary_inventory_info AS ds_broker_inventory_info
ON
  bs.user_id = ds_broker_inventory_info.broker_id


LEFT JOIN
  dw_temp_angejia.dw_broker_summary_wechat_message_daily AS ds_summary_broker_msg
ON
...
LEFT JOIN
  dw_temp_angejia.dw_broker_summary_incoming_telegram AS ds_broker_incoming_telegram
ON
...
CREATE TABLE dw_db.dw_summary_broker_basis_info_daily_20150605 LIKE dw_db.dw_summary_broker_basis_info_daily;

INSERT OVERWRITE TABLE
  dw_db.dw_summary_broker_basis_info_daily_20150605
PARTITION (
  p_dt
)
SELECT
...
MYSQL
  ALTER TABLE
    dw_db.dw_summary_broker_basis_info_daily
  ADD msg_received_wechat_info_daily varchar(255) DEFAULT NULL,
  ADD msg_reply_wechat_info_daily varchar(255) DEFAULT NULL,
  ADD inventory_sham_num varchar(255) DEFAULT NULL,
  ADD broker_info_full_status varchar(255) DEFAULT NULL,
  ADD broker_has_leader varchar(255) DEFAULT NULL,
  ADD call_daily_num varchar(255) DEFAULT NULL,
  ADD call_daily_people_num varchar(255) DEFAULT NULL,
  ADD broker_laidian_genjin_num varchar(255) DEFAULT NULL,
  ADD working_hours varchar(255) DEFAULT NULL;
```
