(function($){
   $.extend({
     "toSelect"  : function(textid,ulid,bgcolor,textcolor,tbObj){ 	//input、ul、li颜色、其他颜色、同步元素
		...
		//修1：加入了同步data-value="+$(this).data('value')+"
		  $table.append($("<tr style='width:" + width + "'></tr>").append($("<td data-value="+$(this).data('value')+" style='height:17px;font-size:13px'>"+$.trim($(this).text())+"</td>")));
		});
		//��div�����������Ƴ��¼�
		$div.append($table);
		...
		$tr.mouseover(function(){
		 ...
		 //修2：同步到input元素
		 if ($tbObj) {
		 	$tbObj.val($(this).children("td").data('value'));
		 }
		  ...
		  $div.hide();
		   ...
		   if(event.keyCode==13){
		     if(window.document.activeElement==$text[0]){
			   ...
			   //修3：同步到input元素，减半事件
			   if ($tbObj) {
			   		$tbObj.val($table.children().children("[selected='selected']").children().data('value'));
			   }
			   $text.blur();
			 }
		   }
