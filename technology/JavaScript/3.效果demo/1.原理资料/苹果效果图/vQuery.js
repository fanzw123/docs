...
function myAddEvent(obj,sEv,fn) {
	...
	if (obj.attachEvent) { //IE 下 (ie下有bug会把绑定事情的对象this指针指向window)
		...
		obj.addEventListener(sEv,function (ev) { //1.接收外部传入的函数，生成ev对象
			if (false == fn.call(obj,ev)) {						//2.执行外部的函数，传入对象，和ev对象。
...
//拖拽框架
VQuery.prototype.drag = function (style) {//
	var i = null;
	...
	function DivMove (oDiv,style) { 	
		oDiv.onmousedown = function (ev) {	//按下
			var oEvent = ev || event;
			var disX = oEvent.clientX - oDiv.offsetLeft;	//鼠标x - 对象当前x = 鼠标距离对象左边框位置
			var disY = oEvent.clientY - oDiv.offsetTop;
			
			oDiv.onmousemove = function (ev) {	//移动
				var oEvent = ev || event;
				var iLeft = oEvent.clientX - disX;	//鼠标x  - 鼠标距离对象左边框位置 = 对象实际移动位置
